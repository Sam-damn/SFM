...
//c'tor
OFFeatureMatcher::OFFeatureMatcher(std::vector<cv::Mat>& imgs_,std::vector<std::vector<cv::KeyPoint> >& imgpts_):imgpts(imgpts_), imgs(imgs_)
{
...
void OFFeatureMatcher::MatchFeatures(int idx_i, int idx_j, vector<DMatch>* matches) {
	...
	if (imgs[idx_i].channels() == 3) {
		cvtColor(imgs[idx_i], prevgray, COLOR_BGR2GRAY);
		cvtColor(imgs[idx_j], gray, COLOR_BGR2GRAY);
	}
	else {
		prevgray = imgs[idx_i];
		gray = imgs[idx_j];
